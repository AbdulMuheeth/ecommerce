...
export const createPaymentIntent = TryCatch(async(req,res,next)=>{
    ...
    if(!amount) return next( new ErrorHandler("Please enter All fields",400))
...
export const newCoupon = TryCatch(async(req,res,next)=>{
    ...
    return res.status(201).json({
        success:true,
        message: `Coupon ${coupon} created succcessfully`
    })
})
...
export const deleteCoupon = TryCatch(async(req,res,next)=>{
    ...
    if(!coupon) return next(new ErrorHandler("Invalid Coupon, Coupon with given ID not found",404))
