...
export const reduceStock = async (orderItems) => {
    orderItems.forEach(async (item) => {
        ...
        if (!product)
            throw new Error("Product with given id is not found");
...
export const getInventories = async ({ categories, productCount, }) => {
    const categoryCount = categories.map((category) => Product.countDocuments({ category }));
    ...
    const inventory = categories.reduce((acc, curr, pos) => {
        return [
            ...acc,
            {
                name: curr,
                count: inventoryCount[pos],
                percentage: Math.round((inventoryCount[pos] / productCount) * 100),
            },
        ];
    }, []);
    return inventory;
